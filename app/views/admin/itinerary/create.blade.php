...
{{-- Web site Title --}}
@section('title')
  {{Config::get('config.site_name')}} | Create new Itinerary
@stop
...
<script type="text/javascript">
	$(document).ready(function($) {
		...
		function post(options) {
			...
			if($('#itinerary-form input[name="title"]').val() == "") {
				var title = "Untitled";
				options.status = 'Draft';
				$('#itinerary-form input[name="title"]').val(title);
			}
			...
			$(".hero-image-upload").imageUpload('appendImages', fd);
			...
			.always(function(e) {
				if(options.refresh) {
					document.location = e.itinerary.edit_url;
				}
		...
		$(".hero-image-upload").imageUpload({
			replace:true,
			type:'Itinerary',
			id:{{isset($itinerary) ? $itinerary->id:'null'}}, 
			path:'assets/content/itinerary',
			property:"heroPhoto",
...
<div class="page-header">
    <h2 class="inline">
      {{ isset($itinerary) ? $itinerary->title : 'Create Itinerary'}}
    </h2>
    <h5 class="inline">
    	@if (isset($itinerary))
...
{{Form::open(['url'=>isset($itinerary)?$itinerary->getPostURL():'/itineraries', 'method'=>isset($itinerary)?'PUT':'POST', 'id'=>'itinerary-form'])}}
<div class="col-md-12">
  	<div class="row">
  		...
  		<div class="row">
			<div class="col-md-6">			
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						 <h3 class="panel-title">Itineraries Details</h3>
					</div>
					...
					<div class="panel-body" id="spots-locations-panel">
			          	...
			          	<div class="form-group">
				            ...
				            <input id="title" name="title" class="form-control" value="{{isset($itinerary) ? $itinerary->title : ''}}" {{( isset($itinerary) && $itinerary->isFavorites() )?'disabled':''}}>
				            <small class="help-block">slug: {{isset($itinerary) ? $itinerary->slug : ''}}</small>
				        </div>
			...
			<div class="col-sm-6">		
						...
						<div class="row">
							<div class="col-md-12">
								...
								<!--<div class="pull-right">
									<button class="btn btn-default" id="preview-btn" data-id="{{isset($itinerary)?$itinerary->id:''}}">Preview</button>
								</div>-->
							</div>
						</div>
						<br>
							...
							<div class="col-md-12">
								<div class="pull-left">
									@if (isset($itinerary) && $itinerary->isFavorites()===false)
										...
										@else 
											<a id="itinerary-delete-btn" data-id="{{$itinerary->id}}" class="pull-right btn btn-danger btn-default">Delete</a>
										@endif
									@endif
								</div>
								<div class="pull-right">
				...
				<div class="panel panel-default">
					...
					@if (empty($itinerary) || isset($itinerary) && $itinerary->hasHeroImage() == false)
					...
					<ul class="list-group files-list" id="hero-filelist">
						@if (isset($itinerary) && $itinerary->hasHeroImage())
							@include('admin.partials.photo-list-item', array('id' => $itinerary->id, 'image'=>$itinerary->heroPhoto, 'title'=>$itinerary->heroPhoto->getName(), 'multiple'=>false, 'replace'=>true, 'target'=>'hero-image-upload', 'preview'=>'#hero-filelist'))						
						@endif
					</ul>
				</div>
				{{-- -------------------------------------------------------------------------- --}}
